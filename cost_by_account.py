...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Summarize AWS costs by account and export to CSV."
    )
    parser.add_argument(
        "--input",
    ...
    parser.add_argument(
        "--output",
        default="cost_by_account.csv",
        help="Output CSV filename (default: cost_by_account.csv).",
    )
    parser.add_argument(
        "--gcp-key",
        default="key.json",
    ...
    parser.add_argument(
        "--sheet-tab",
        default="cost_by_account",
        help="Google Sheet tab name to write data into (default: cost_by_account).",
    )
...
def write_summary(output_path, totals, unit):
    ...
    with open(output_path, "w", newline="", encoding="utf-8") as csvfile:
        ...
        writer.writerow(["account_id", "account_name", "total_amount", "unit"])
        for (account_id, account_name), amount in sorted_rows:
            formatted_amount = f"{currency_prefix}{amount:,.2f}"
            writer.writerow([account_id, account_name, formatted_amount, unit])
